...
public class QuestionsDAOTest {
	...
	StudentStatusDAO moduleDao = new StudentStatusDAO();
	...
	public void experimentList() {
		Map<Module, List<Question>> mapper  = questions.getLabQuestions();

		System.out.println("Length : "+mapper.size());
		for(Map.Entry<Module, List<Question>> map : mapper.entrySet() ) {
			System.out.println("S : "+map.getKey().getName());
			System.out.println("Val : "+map.getValue().size());
			Iterator iterate = map.getValue().iterator();
			
			while(iterate.hasNext()) {
				Question ex = (Question) iterate.next();
