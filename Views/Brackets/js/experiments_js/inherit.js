function addCommonHint(tId, v) {
    var h =  '<li id="'+tId+'">' +
                   '<ul>' +
                       ...
                       '<li><input type="file" id="'+tId+'_Fs" class="hint_Files" name="hints_'+tId+'"></li>    ' + 
                  ' </ul>' +
               '</li>';
...
var expId, oList, mList;
function OMList(expId, oList, mList) {
    this.expId = expId;
    this.oList = oList;
    this.mList = mList;
}
...
function sendOrder() {
    sendURL('read_order_modules', new OMList(sessionStorage.getItem("newexperimentId"), orderList, modulesList), demo);
}
